...
import com.tightvnc.vncviewer.io.SessionRecorder;
import com.tightvnc.vncviewer.io.SocketFactory;
...
public class RfbProto
{
    ...
    public final static String StandardVendor = "STDV", TridiaVncVendor = "TRDV", TightVncVendor = "TGHT";

    // Security types
    public final static int SecTypeInvalid = 0, SecTypeNone = 1, SecTypeVncAuth = 2, SecTypeTight = 16;
    ...
    public final static String SigAuthNone = "NOAUTH__", SigAuthVNC = "VNCAUTH_", SigAuthUnixLogin = "ULGNAUTH";

    // VNC authentication results
    public final static int VncAuthOK = 0, VncAuthFailed = 1, VncAuthTooMany = 2;
    ...
    public final static int PREFIX_TIGHT = 0xFC;
    ...
    public final static int EnableVideoHandling = 153;

    public final static String SigEnableVideoHandling = "VD_ENABL";

    // Supported encodings and pseudo-encodings
    public final static int EncodingRaw = 0, EncodingCopyRect = 1, EncodingRRE = 2, EncodingCoRRE = 4,
                    EncodingHextile = 5, EncodingZlib = 6, EncodingTight = 7, EncodingZRLE = 16,
                    EncodingCompressLevel0 = 0xFFFFFF00, EncodingQualityLevel0 = 0xFFFFFFE0,
                    EncodingXCursor = 0xFFFFFF10, EncodingRichCursor = 0xFFFFFF11, EncodingPointerPos = 0xFFFFFF18,
                    EncodingLastRect = 0xFFFFFF20, EncodingNewFBSize = 0xFFFFFF21;

    public final static String SigEncodingRaw = "RAW_____", SigEncodingCopyRect = "COPYRECT",
                    SigEncodingRRE = "RRE_____", SigEncodingCoRRE = "CORRE___", SigEncodingHextile = "HEXTILE_",
                    SigEncodingZlib = "ZLIB____", SigEncodingTight = "TIGHT___", SigEncodingZRLE = "ZRLE____",
                    SigEncodingCompressLevel0 = "COMPRLVL", SigEncodingQualityLevel0 = "JPEGQLVL",
                    SigEncodingXCursor = "X11CURSR", SigEncodingRichCursor = "RCHCURSR",
                    SigEncodingPointerPos = "POINTPOS", SigEncodingLastRect = "LASTRECT",
    ...
    private boolean zlibWarningShown;

    private boolean tightWarningShown;
    ...
    private int clientMajor, clientMinor;

    private boolean protocolTightVNC;

    private CapsContainer tunnelCaps, authCaps;

    private CapsContainer serverMsgCaps, clientMsgCaps;
    ...
    public RfbProto( final String h, final int p, final VncViewer v )
        throws IOException
    {
        viewer = v;
        host = h;
        ...
        else
        {
            try
            {
                ...
                final Class<SocketFactory> factoryClass =
                    (Class<SocketFactory>) Class.forName( viewer.getSocketFactory() );
                ...
                {
                    sock = factory.createSocket( host, port, viewer.getMainArgs() );
                }
            }
        ...
        throws Exception
    {
        ...
        if ( ( b[0] != 'R' ) || ( b[1] != 'F' ) || ( b[2] != 'B' ) || ( b[3] != ' ' ) || ( b[4] < '0' )
            || ( b[4] > '9' ) || ( b[5] < '0' ) || ( b[5] > '9' ) || ( b[6] < '0' ) || ( b[6] > '9' ) || ( b[7] != '.' )
            || ( b[8] < '0' ) || ( b[8] > '9' ) || ( b[9] < '0' ) || ( b[9] > '9' ) || ( b[10] < '0' )
            || ( b[10] > '9' ) || ( b[11] != '\n' ) )
        {
            throw new Exception( "Host " + host + " port " + port + " is not an RFB server" );
        }

        setServerMajor( ( b[4] - '0' ) * 100 + ( b[5] - '0' ) * 10 + ( b[6] - '0' ) );
        setServerMinor( ( b[8] - '0' ) * 100 + ( b[9] - '0' ) * 10 + ( b[10] - '0' ) );
    ...
    public int negotiateSecurity()
        throws Exception
    {
        return ( getClientMinor() >= 7 ) ? selectSecurityType() : readSecurityType();
    }
    ...
    public int selectSecurityType()
        throws Exception
    {
        int secType = SecTypeInvalid;

        // Read the list of secutiry types.
        final int nSecTypes = readU8();
        if ( nSecTypes == 0 )
        {
            readConnFailedReason();
            return SecTypeInvalid; // should never be executed
        }
        final byte[] secTypes = new byte[nSecTypes];
        readFully( secTypes );
    ...
    public void authenticateVNC( String pw )
        throws Exception
    {
    ...
    public void readSecurityResult( final String authType )
        throws Exception
    {
        final int securityResult = readU32();
    ...
    public void readConnFailedReason()
        throws Exception
    {
        final int reasonLen = readU32();
    ...
    public void initCapabilities()
    {
        ...
        setClientMsgCaps( new CapsContainer() );
        ...
        serverMsgCaps.add( EndOfContinuousUpdates, TightVncVendor, SigEndOfContinuousUpdates,
                           "End of continuous updates notification" );

        // Supported non-standard client-to-server messages
        getClientMsgCaps().add( EnableContinuousUpdates, TightVncVendor, SigEnableContinuousUpdates,
                                "Enable/disable continuous updates" );
        getClientMsgCaps().add( VideoRectangleSelection, TightVncVendor, SigVideoRectangleSelection,
                                "Select a rectangle to be treated as video" );
        getClientMsgCaps().add( VideoFreeze, TightVncVendor, SigVideoFreeze, "Disable/enable video rectangle" );
        getClientMsgCaps().add( EnableVideoHandling, TightVncVendor, SigEnableVideoHandling,
                                "Disable/enable special video handling" );
    ...
    public void setupTunneling()
        throws IOException
    {
        final int nTunnelTypes = readU32();
        if ( nTunnelTypes != 0 )
        {
    ...
    public int negotiateAuthenticationTight()
        throws Exception
    {
        final int nAuthTypes = readU32();
        if ( nAuthTypes == 0 )
        {
            return AuthNone;
        }
        ...
        {
            final int authType = authCaps.getByOrder( i );
    ...
    public void readCapabilityList( final CapsContainer caps, final int count )
        throws IOException
    {
        int code;
    ...
    private boolean bigEndian, trueColour;

    private int redMax, greenMax, blueMax, redShift, greenShift, blueShift;

    public void readServerInit()
        throws IOException
    {
        setFramebufferWidth( readU16() );
        setFramebufferHeight( readU16() );
        bitsPerPixel = readU8();
        depth = readU8();
        bigEndian = ( readU8() != 0 );
        trueColour = ( readU8() != 0 );
        ...
        readFully( pad );
        final int nameLength = readU32();
        final byte[] name = new byte[nameLength];
        readFully( name );
        setDesktopName( new String( name ) );

        // Read interaction capabilities (TightVNC protocol extensions)
        if ( protocolTightVNC )
        {
            final int nServerMessageTypes = readU16();
            final int nClientMessageTypes = readU16();
            final int nEncodingTypes = readU16();
            readU16();
            readCapabilityList( serverMsgCaps, nServerMessageTypes );
            readCapabilityList( getClientMsgCaps(), nClientMessageTypes );
            readCapabilityList( encodingCaps, nEncodingTypes );
        }

        if ( !getClientMsgCaps().isEnabled( EnableContinuousUpdates ) )
        {
            viewer.getOptions()
                  .disableContUpdates();
        }
    ...
    public void startSession( final String fname )
        throws IOException
    {
        ...
        rec.writeShortBE( getFramebufferWidth() );
        rec.writeShortBE( getFramebufferHeight() );
        final byte[] fbsServerInitMsg =
            { 32, 24, 0, 1, 0, (byte) 0xFF, 0, (byte) 0xFF, 0, (byte) 0xFF, 16, 8, 0, 0, 0, 0 };
        rec.write( fbsServerInitMsg );
        rec.writeIntBE( getDesktopName().length() );
        rec.write( getDesktopName().getBytes() );
    ...
    public void setFramebufferSize( final int width, final int height )
    {
        ...
        throws IOException
    {
        ...
        if ( protocolTightVNC && msgType == PREFIX_TIGHT )
        {
            ...
            readFully( buf );
            msgType = (int) ( (long) PREFIX_TIGHT << 24 | (long) buf[0] << 16 | (long) buf[1] << 8 | buf[2] );
        }
    ...
    public void readFramebufferUpdate()
        throws IOException
    {
        skipBytes( 1 );
        setUpdateNRects( readU16() );
        ...
        if ( rec != null )
        {
            ...
            rec.writeByte( 0 );
            rec.writeShortBE( getUpdateNRects() );
        }
    ...
    public boolean isRealDecoderEncoding( final int encoding )
    {
    ...
    public void readFramebufferUpdateRectHdr()
        throws Exception
    {
        setUpdateRectX( readU16() );
        setUpdateRectY( readU16() );
        setUpdateRectW( readU16() );
        setUpdateRectH( readU16() );
        setUpdateRectEncoding( readU32() );

        if ( getUpdateRectEncoding() == EncodingZlib || getUpdateRectEncoding() == EncodingZRLE
            || getUpdateRectEncoding() == EncodingTight )
        {
        ...
        if ( rec != null )
        {
            ...
            rec.writeShortBE( getUpdateRectX() );
            rec.writeShortBE( getUpdateRectY() );
            rec.writeShortBE( getUpdateRectW() );
            rec.writeShortBE( getUpdateRectH() );
            ...
            if ( ( ( !isRealDecoderEncoding( getUpdateRectEncoding() ) ) ) && ( rec != null ) )
            {
                rec.writeIntBE( getUpdateRectEncoding() );
            }
        }

        if ( getUpdateRectEncoding() < 0 || getUpdateRectEncoding() > MaxNormalEncoding )
        {
            return;
        }

        if ( getUpdateRectX() + getUpdateRectW() > getFramebufferWidth()
            || getUpdateRectY() + getUpdateRectH() > getFramebufferHeight() )
        {
            throw new Exception( "Framebuffer update rectangle too large: " + getUpdateRectW() + "x" + getUpdateRectH()
                + " at (" + getUpdateRectX() + "," + getUpdateRectY() + ")" );
        }
    }
    ...
    public void writeFramebufferUpdateRequest( final int x, final int y, final int w, final int h,
    ...
    public void writeSetPixelFormat( final int bitsPerPixel, final int depth, final boolean bigEndian,
                                     final boolean trueColour, final int redMax, final int greenMax, final int blueMax,
    ...
    public void writeFixColourMapEntries( final int firstColour, final int nColours, final int[] red,
                                          final int[] green, final int[] blue )
        throws IOException
    {
        final byte[] b = new byte[6 + nColours * 6];
    ...
    public void writeSetEncodings( final int[] encs, final int len )
        throws IOException
    {
    ...
    public void writeClientCutText( final String text )
        throws IOException
    {
    ...
    public void writePointerEvent( final MouseEvent evt )
        throws IOException
    {
        writePointerEvent( evt, false );
    }

    public void writePointerEvent( final MouseEvent evt, final boolean down )
        throws IOException
    {
        writePointerEvent( evt, down, false, 0 );
    }

    public void writePointerEvent( final MouseEvent evt, final boolean down, final boolean wheelEvent,
    ...
    public void writeKeyEvent( final KeyEvent evt )
        throws IOException
    {
        ...
        final boolean down = ( evt.getID() == KeyEvent.KEY_PRESSED );
    ...
    public void writeKeyEvent( final int keysym, final boolean down )
    {
    ...
    public void writeModifierKeyEvents( final int newModifiers )
    {
    ...
    public void tryEnableContinuousUpdates( final int x, final int y, final int w, final int h )
        throws IOException
    {
        if ( !getClientMsgCaps().isEnabled( EnableContinuousUpdates ) )
        {
    ...
    public void tryDisableContinuousUpdates()
        throws IOException
    {
        if ( !getClientMsgCaps().isEnabled( EnableContinuousUpdates ) )
        {
        ...
        final byte[] b = { (byte) EnableContinuousUpdates, 0, 0, 0, 0, 0, 0, 0, 0, 0 };
        os.write( b );
    ...
    /**
     * Send a rectangle selection to be treated as video by the server (but only if VideoRectangleSelection message is
     * supported by the server).
     * 
     * @param rect specifies coordinates and size of the rectangule.
     * @throws java.io.IOException
     */
    public void trySendVideoSelection( Rectangle rect )
        throws IOException
    {
        if ( !getClientMsgCaps().isEnabled( VideoRectangleSelection ) )
        {
    ...
    public void trySendVideoFreeze( final boolean freeze )
        throws IOException
    {
        if ( !getClientMsgCaps().isEnabled( VideoFreeze ) )
        {
    ...
    public void trySendVideoEnable( final boolean enable )
        throws IOException
    {
        if ( !getClientMsgCaps().isEnabled( EnableVideoHandling ) )
        {
    ...
    public final int readU8()
        throws IOException
    {
        final int r = is.readUnsignedByte();
        numBytesRead++;
        return r;
    }
    ...
    public final int readU16()
        throws IOException
    {
        final int r = is.readUnsignedShort();
        numBytesRead += 2;
        return r;
    }
    ...
    public final int readU32()
        throws IOException
    {
        final int r = is.readInt();
        numBytesRead += 4;
        return r;
    }

    public CapsContainer getClientMsgCaps()
    {
        return clientMsgCaps;
    }

    public void setClientMsgCaps( final CapsContainer clientMsgCaps )
    {
        this.clientMsgCaps = clientMsgCaps;
    }
    ...
    public void setServerMajor( final int serverMajor )
    {
    ...
    public void setServerMinor( final int serverMinor )
    {
    ...
    public void setClientMajor( final int clientMajor )
    {
    ...
    public void setClientMinor( final int clientMinor )
    {
    ...
    public void setFramebufferWidth( final int framebufferWidth )
    {
    ...
    public void setFramebufferHeight( final int framebufferHeight )
    {
    ...
    public void setInNormalProtocol( final boolean inNormalProtocol )
    {
        this.inNormalProtocol = inNormalProtocol;
    }

    public int getUpdateNRects()
    {
        return updateNRects;
    }

    public void setUpdateNRects( final int updateNRects )
    {
        this.updateNRects = updateNRects;
    }

    public int getUpdateRectX()
    {
        return updateRectX;
    }

    public void setUpdateRectX( final int updateRectX )
    {
        this.updateRectX = updateRectX;
    }

    public int getUpdateRectY()
    {
        return updateRectY;
    }

    public void setUpdateRectY( final int updateRectY )
    {
        this.updateRectY = updateRectY;
    }

    public int getUpdateRectH()
    {
        return updateRectH;
    }

    public void setUpdateRectH( final int updateRectH )
    {
        this.updateRectH = updateRectH;
    }

    public int getUpdateRectW()
    {
        return updateRectW;
    }

    public void setUpdateRectW( final int updateRectW )
    {
        this.updateRectW = updateRectW;
    }

    public int getUpdateRectEncoding()
    {
        return updateRectEncoding;
    }

    public void setUpdateRectEncoding( final int updateRectEncoding )
    {
        this.updateRectEncoding = updateRectEncoding;
    }

    public SessionRecorder getSessionRecorder()
    {
        return rec;
    }
}
