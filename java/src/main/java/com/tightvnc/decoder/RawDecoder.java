...
public class RawDecoder
{
    ...
    public RawDecoder( final Graphics g, final RfbInputStream is )
    {
        setGraphics( g );
        setRfbInputStream( is );
    }

    public RawDecoder( final Graphics g, final RfbInputStream is, final int frameBufferW, final int frameBufferH )
    {
        setGraphics( g );
        setRfbInputStream( is );
    ...
    public void setRfbInputStream( final RfbInputStream is )
    {
        rfbis = is;
    }
    ...
    public void setFrameBufferSize( final int w, final int h )
    {
        framebufferWidth = w;
        framebufferHeight = h;
    }
    ...
    public void setDataOutputStream( final DataOutput os )
    {
        dos = os;
    }
    ...
    public void handleRect( final int x, final int y, final int w, final int h )
    ...
    protected void handleUpdatedPixels( final int x, final int y, final int w, final int h )
    {
    ...
    public void update()
    {
        ...
        final int fbWidth = framebufferWidth;
        final int fbHeight = framebufferHeight;

        if ( bytesPerPixel == 1 )
        {
            pixels24 = null;
            pixels8 = new byte[fbWidth * fbHeight];
            pixelsSource = new MemoryImageSource( fbWidth, fbHeight, getColorModel8(), pixels8, 0, fbWidth );
        }
        else
        {
            pixels8 = null;
        ...
        rawPixelsImage = Toolkit.getDefaultToolkit()
                                .createImage( pixelsSource );
    }
    ...
    protected void enableEncodingRecordWritting( final boolean enable )
    {
